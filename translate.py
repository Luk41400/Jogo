...
# Replace QUIZ_LIST
quiz_start = html.find("const QUIZ_LIST = {")
quiz_end = html.find("};", quiz_start) + 2

html = html[:quiz_start] + quiz_list_str + html[quiz_end:]
...
html = html.replace('• <b>W / ⬆️</b>: Accelerate (Start movement)<br>', '')
html = html.replace('• <b>S / ⬇️</b>: Brake / Reverse Gear<br>', '')
html = html.replace('• <b>A - D / ⬅️ - ➡️</b>: Steering Controls<br>', '')
html = html.replace('• <b>SPACE</b>: Activate <b>NITRO</b> (Burst speed!)<br>', '')
html = html.replace('• <b>QUIZ</b>: Solve in 10s to gain $ and HP!<br>', '')
html = html.replace('• <b>GARAGE</b>: Buy cars in order to improve stats.', '')
...
sync_func_start = html.find("function syncHUD() {")
...
# Fix Garage logic
gar_logic = '''const card = document.createElement('div'); card.style = "background:#000; padding:12px; margin-bottom:8px; border-radius:8px; display:flex; justify-content:space-between; align-items:center; border:1px solid " + (sel ? 'var(--safety-cyan)' : '#333');
...
html = html.replace('''const card = document.createElement('div'); card.style = "background:#000; padding:12px; margin-bottom:8px; border-radius:8px; display:flex; justify-content:space-between; align-items:center; border:1px solid " + (sel ? 'var(--safety-cyan)' : '#333');
                card.innerHTML = `<div><b>${v.n}</b><br><small>${has ? 'OWNED' : '$' + v.p}</small></div><button class="btn" style="margin:0; padding:6px 12px; font-size:0.8rem; ${(!has && !unlock) ? 'opacity:0.5;' : ''}">${has ? (sel ? 'EQUIPPED' : 'SELECT') : (unlock ? 'BUY' : 'LOCKED')}</button>`;''', gar_logic)
...
"""
html = html.replace('CURRENT BALANCE: <span id="m-menu"', lang_html + 'CURRENT BALANCE: <span id="m-menu"')
...
"""
html = html.replace('function loop(now)', change_lang_fn + 'function loop(now)')
